<?php
// Debug why no media files are found for Quote #69 - FIXED VERSION
header('Content-Type: text/plain');
echo "=== DEBUGGING MEDIA FILES FOR QUOTE #69 (FIXED) ===\n";
...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, name, email, quote_status FROM quotes WHERE id = 69");
    $stmt->execute();
    $quote = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if ($quote) {
        echo "✅ Quote found: ID={$quote['id']}, Name={$quote['name']}, Status={$quote['quote_status']}\n";
    } else {
    ...
    // Get column names for quotes table
    ...
    // Get column names for uploaded_files table
